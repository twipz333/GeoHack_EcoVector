    ...
    if request.method == 'GET':
        if id:
            try:
                ...
                return JsonResponse({'error': str(ex), 'content':request.data}, status=status.HTTP_404_NOT_FOUND)
        else:
        ...
        if data:
            try:
                ...
                return JsonResponse(data={'error': str(ex), 'content':request.data}, status=status.HTTP_400_BAD_REQUEST)
            
            return JsonResponse(UserSerializer(user).data, status=status.HTTP_201_CREATED)
        
        return JsonResponse(data={'error':'Missing arguments!', 'content':request.data}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if data:
            ...
            try:
                ...
                return JsonResponse(data={'error':str(ex), 'content':request.data}, status=status.HTTP_404_NOT_FOUND)
        ...
        if event_id:
            ...
            try:
                ...
                if data.get('event_id'):
                    ...
                    return JsonResponse(data={'error': 'Missing argument event_id', 'content':request.data}, status=status.HTTP_400_BAD_REQUEST)
            except User.DoesNotExist as ex:
                return JsonResponse(data={'error':str(ex), 'content':request.data}, status=status.HTTP_404_NOT_FOUND)
            except Event.DoesNotExist as ex:
                return JsonResponse(data={'error':str(ex), 'content':request.data}, status=status.HTTP_404_NOT_FOUND)
    ...
    if request.method == 'GET':
        ...
        if data:
            try:
                ...
                return Response(data={'error': str(ex), 'content':request.data}, status=status.HTTP_400_BAD_REQUEST)
            except User.DoesNotExist as ex:
                return Response(data={'error': str(ex), 'content':request.data}, status=status.HTTP_404_NOT_FOUND)
            except Event.DoesNotExist as ex:
                return Response(data={'error': str(ex), 'content':request.data}, status=status.HTTP_404_NOT_FOUND)
